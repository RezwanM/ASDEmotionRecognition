#************************************************************************************
# Rezwan Matin
# Speech Emotion Recognition using SVM and RAVDESS+TESS corpora (w/ added noise)
# Filename: SVM_RAVDESS_TESS_2.py
# Date: 01/29/20
#
# Objective:
# Testing RAVDESS+TESS data with noise.
#
#*************************************************************************************
...
path = r'C:\Books\Texas State Books\Fall 2019\Thesis A\Corpus\Simulated\RAVDESS+TESS\All'
...
y = df_upsampled.iloc[0:4144, 27].values
# Extract features 'buying' and 'safety' in a vector X. Indexing from 0
X = df_upsampled.iloc[0:4144, list(range(27))].values
print(y)
...
svm = SVC(kernel='rbf', C=3.80, gamma=0.01, random_state=1)
