...
# Rezwan Matin
# Thesis B
# Filename: RNN_Parselmouth_TESS_Clean.py
# Date: 10/24/20
#
# Features:
...
y = df.iloc[0:2800, 15*median_num_frames].values
# Extract features in a matrix X. Indexing from 0
X = df.iloc[0:2800, list(range(15*median_num_frames))].values
print(y)
...
plt.title('RNN_P.GeMAPS_TESS_Clean')
plt.grid()
plt.ylabel('accuracy')
plt.xlabel('epoch')
...
fig.savefig('Accuracy_Curves_RNN_Parselmouth_TESS_Clean.png')
...
#plt.show()
# Save plot as PNG file
fig.savefig('Loss_Curves_RNN_Parselmouth_TESS_Clean.png')
