...
# Rezwan Matin
# Thesis B
# Filename: MLP_Parselmouth_CREMA-D_Clean_Noise.py
# Date: 10/24/20
#
# Features:
...
df = pd.read_csv(r'/home/r_m727/All_Files/Corpus/Simulated/CREMA-D/CREMA-D_Parselmouth_Clean_Noise.csv', header=0, index_col=0)
...
df['Emotion'].replace({1.0 : 1.0, 3.0 : 2.0, 4.0 : 3.0, 5.0 : 4.0, 6.0 : 5.0, 7.0 : 6.0}, inplace=True)
...
y_less = df_less.iloc[0:12710, 36].values
# Extract features of majority classes.
X_less = df_less.iloc[0:12710, list(range(36))].values
print(y_less)
...
plt.title('MLP_P.GeMAPS_CREMA-D_Clean_Noise')
plt.grid()
plt.ylabel('accuracy')
plt.xlabel('epoch')
...
fig.savefig('Accuracy_Curves_MLP_Parselmouth_CREMA-D_Clean_Noise.png')
...
#plt.show()
# Save plot as PNG file
fig.savefig('Loss_Curves_MLP_Parselmouth_CREMA-D_Clean_Noise.png')
