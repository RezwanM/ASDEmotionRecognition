...
# Rezwan Matin
# Thesis B
# Filename: SVM_Parselmouth_Complete_Clean_Noise.py
# Date: 10/24/20
#
# Features:
...
for i in range(0,10):
	...
	svm = SVC(kernel='rbf', C=10, gamma=0.01, random_state=None)
	...
	sum_train = sum_train + svm.score(X_train_centered, y_train)
	sum_val = sum_val + svm.score(X_val_centered, y_val)


avg_train = sum_train / 10
avg_val = sum_val / 10

print('Average Training Accuracy: %.2f%%' % (avg_train * 100))
print('Average Validation Accuracy: %.2f%%' % (avg_val * 100))
