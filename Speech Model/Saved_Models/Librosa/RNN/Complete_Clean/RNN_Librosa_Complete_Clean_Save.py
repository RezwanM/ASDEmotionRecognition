...
# Rezwan Matin
# Thesis B
# Filename: RNN_Librosa_Complete_Clean_Save.py
# Date: 10/24/20
#
# Features:
# 36 features - 26 MFCCs, 7 spectral contrast, 2 poly features, and 1 RMS.
#
# Saving mean and standard deviation arrays of features to NPY files for standardizing real-time data!
#*************************************************************************************
...
df = pd.read_csv(r'/home/r_m727/All_Files/Corpus/Simulated/RAVDESS+TESS+CREMA-D/RAVDESS+TESS+CREMA-D_Librosa_Clean_RNN.csv', header=0, index_col=0)
...
y_less = df_less.iloc[0:9315, 36*median_num_frames].values
# Extract features of majority classes.
X_less = df_less.iloc[0:9315, list(range(36*median_num_frames))].values
print(y_less)
...
X_train_surprised = train_surprised_resampled[:, 0:36*median_num_frames]
X_val_surprised = val_surprised_resampled[:, 0:36*median_num_frames]
X_test_surprised = test_surprised_resampled[:, 0:36*median_num_frames]
y_train_surprised = train_surprised_resampled[:, 36*median_num_frames]
y_val_surprised = val_surprised_resampled[:, 36*median_num_frames]
y_test_surprised = test_surprised_resampled[:, 36*median_num_frames]
...
np.save('mean_feat_rnn_librosa_clean.npy', mean_vals)
np.save('std_feat_rnn_librosa_clean.npy', std_val)
...
plt.title('RNN_Custom_Complete_Clean')
plt.grid()
plt.ylabel('accuracy')
plt.xlabel('epoch')
...
fig.savefig('Accuracy_Curves_RNN_Librosa_Complete_Clean.png')
...
#plt.show()
# Save plot as PNG file
fig.savefig('Loss_Curves_RNN_Librosa_Complete_Clean.png')

# Save the model as an h5 file
model.save('RNN_Librosa_Complete_Clean.h5')
