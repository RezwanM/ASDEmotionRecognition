...
# Rezwan Matin
# Thesis B
# Filename: RNN_Librosa_TESS_Clean.py
# Date: 10/24/20
#
# Features:
...
df = pd.read_csv(r'/home/r_m727/All_Files/Corpus/Simulated/TESS/TESS_Librosa_Clean_RNN.csv', header=0, index_col=0)
...
y = df.iloc[0:2800, 36*median_num_frames].values
# Extract features in a matrix X. Indexing from 0
X = df.iloc[0:2800, list(range(36*median_num_frames))].values
print(y)
...
plt.title('RNN_Custom_TESS_Clean')
plt.grid()
plt.ylabel('accuracy')
plt.xlabel('epoch')
...
fig.savefig('Accuracy_Curves_RNN_Librosa_TESS_Clean.png')
...
#plt.show()
# Save plot as PNG file
fig.savefig('Loss_Curves_RNN_Librosa_TESS_Clean.png')
